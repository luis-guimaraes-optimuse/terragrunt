...
Each entry becomes a section holding the metadata the catalog user interface shows for it, the command to run to get a copy of the component, and the component's README. Sections are written as entries are discovered, so the document is readable while the remaining repositories are still loading.
