...
- `version` (attribute): A version constraint used to resolve a `tfr://` registry module, as an alternative to pinning an exact version in the source URL.

  <Aside type="tip" title="Experimental">
    The `version` attribute is gated behind the `version-attribute` experiment. Enable it with `--experiment version-attribute` (or `TG_EXPERIMENT=version-attribute`) until it graduates to general availability. See the [experiment documentation](/reference/experiments/active#version-attribute).
  </Aside>

  Terragrunt resolves the constraint against the registry's list-versions endpoint and downloads the highest published version that satisfies it. The constraint syntax matches the `version` argument on OpenTofu and Terraform `module` blocks:
  ...
  - The `version` attribute applies to `tfr://` sources only. Setting it alongside a Git, local, or other source is an error.
  - Do not also pin `?version=` inline on the same source. Specify the version in the attribute or the URL, not both, or Terragrunt reports an error.
  - The `--source` and `TG_SOURCE` overrides accept an exact version pin only (for example `?version=3.4.0`); a constraint there is an error.
  - A resolved constraint is cached by source URL. A newly published version that matches the constraint is only picked up after you re-resolve with `--source-update`.
  - Prereleases follow OpenTofu and Terraform: a prerelease is eligible only when the constraint itself names one (for example `>= 2.0.0-rc1`).
...
**Purpose**: Inject dependencies and configuration overrides into a unit or stack at generation time.
**Use case**: When units in a stack depend on each other (or on a sibling stack), or when a stack needs to patch a generated component with configuration that doesn't belong in the catalog source.
**Result**: Generates a `terragrunt.autoinclude.hcl` file next to the unit's `terragrunt.hcl` (for `unit` blocks), or a `terragrunt.autoinclude.stack.hcl` file next to the nested stack's `terragrunt.stack.hcl` (for `stack` blocks). The generated file is automatically merged into the component's configuration whenever the component is parsed.

The `autoinclude` block takes no label and has no arguments of its own. Its body is the configuration to inject. In an `autoinclude` on a `unit` block, the body can contain:

- `dependency` blocks. The `config_path` attribute is resolved at generation time, typically from the `unit.<name>.path` and `stack.<name>.path` variables described below. All other attributes (`mock_outputs`, `mock_outputs_allowed_terraform_commands`, etc.) work as they do in a regular [`dependency`](#dependency) block.
- The `inputs` attribute.
- Any other block that is valid in a unit configuration: `feature`, `errors`, `generate`, `remote_state`, etc.

Three things are rejected with an error:

- A `locals` block fails generation. Declare locals at the stack level in `terragrunt.stack.hcl` instead, where they resolve uniformly at generate time.
- The `values` attribute fails when the generated file is parsed, because the file is read as regular unit configuration and `values` is not a valid argument there. Pass per-unit values through the `values` attribute of the `unit` block instead.
- A nested `autoinclude` block fails generation.
...
Expressions in the autoinclude body resolve at generation time, in the context of the stack file: `local.*`, `values.*`, `unit.<name>.path`, `stack.<name>.path`, and function calls all become literals in the generated file. Because functions evaluate against the stack file rather than the unit, directory functions report the stack file's location (`get_terragrunt_dir()` returns the stack file's directory, and `path_relative_to_include()` returns `"."`). Only `dependency.*` references are kept verbatim, because dependency outputs exist only when the unit runs.

### Component path variables

With the experiment enabled, `terragrunt.stack.hcl` files expose a `unit.<name>.path` and `stack.<name>.path` variable for each declared component. They resolve to the generated directory of that component, following the same layout `stack generate` produces (including the effect of `no_dot_terragrunt_stack` on the referenced block). In a generated dependency, `config_path` is written relative to the unit's directory.

These variables are available inside `autoinclude` blocks and in the `values` attribute of `unit` and `stack` blocks, so a parent stack can pass a component's path down to a child stack. Only components declared in the same stack file are visible; referencing an undeclared name fails generation.

### Merge behavior

A generated `terragrunt.autoinclude.hcl` merges into the unit's configuration using the same default strategy as a regular [`include`](#include): a shallow merge, with the autoinclude winning. Top-level keys from the unit and the autoinclude combine; on a conflict, the autoinclude's value replaces the unit's rather than deep merging nested maps. `inputs` maps combine key by key at the top level. A `dependency` block with the same name as one in the unit's own `terragrunt.hcl` replaces it wholesale.

The merge applies to any unit with a sibling `terragrunt.autoinclude.hcl` while the experiment is enabled. `stack generate` normally writes this file; treat it as a generated artifact rather than something to edit by hand.

### Depending on a stack

A dependency's `config_path` can point at a stack directory via `stack.<name>.path`. The dependency's outputs aggregate the outputs of every unit in that stack, keyed by unit name:

```hcl
# terragrunt.stack.hcl

stack "network" {
  source = "../catalog/stacks/network"
  path   = "network"
}
...
Here the `network` stack contains a unit named `vpc` with a `vpc_id` output, so the dependent unit reads it as `dependency.network.outputs.vpc.vpc_id`. During `run --all`, a dependency on a stack expands to a dependency on each of the stack's units, so the dependent unit waits for all of them to finish.

### Stack-level autoinclude

An `autoinclude` on a `stack` block generates `terragrunt.autoinclude.stack.hcl` next to the nested stack's `terragrunt.stack.hcl`. Its body may declare only `unit` and `stack` blocks, which are injected into the generated stack. An injected block whose name matches an existing `unit` or `stack` overrides that block wholesale; a new name adds a component.
...
stack "networking" {
  ...
  autoinclude {
    unit "extra" {
      source = "${get_repo_root()}/catalog/units/extra"
      path   = "extra"
    }
  }
}
...
A `dependency` block is rejected in a stack-level autoinclude, because stacks themselves have no dependencies. Declare the dependency inside the target unit's own `autoinclude`.

### Limitations

- Stacks are opaque dependency boundaries. `unit.<name>.path` and `stack.<name>.path` only see components declared in the same stack file, so an autoinclude cannot target a unit inside another stack. Depend on the whole stack instead, or pass a component's path down to the child stack through `values`.
- An `autoinclude` block cannot contain another `autoinclude` block. A `unit` or `stack` block injected by a stack-level autoinclude may declare its own `autoinclude`; it resolves during the next generation pass.
- Only HCL stack files support `autoinclude`. A stack file written as JSON fails generation if it declares one.
