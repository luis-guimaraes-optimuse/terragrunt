...
Both carry the same information, and neither is interactive. Scaffolding a module and copying a unit or stack stay in the TUI.
...
| Install | `terragrunt scaffold 'github.com/acme/infrastructure-modules//modules/vpc'` |
...
The description leads the section as prose, and the rest of the entry follows as a table. The rows hold the fields described under [Entry structure](#entry-structure), and the `Install` row reports how to get a copy of the component: `terragrunt scaffold` for a module or template, and copying the files for a unit or stack, which is what the TUI does when you press `s`. `terragrunt scaffold` does not install a unit or a stack; it reads the source as an OpenTofu/Terraform module and generates a `terragrunt.hcl` pointing at it.
