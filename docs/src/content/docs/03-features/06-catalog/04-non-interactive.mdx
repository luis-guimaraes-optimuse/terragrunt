...
The description leads the section as prose, and the rest of the entry follows as a table. The rows hold the fields described under [Entry structure](#entry-structure). The `Component source` row is what you hand to [`terragrunt scaffold`](/features/catalog/scaffold) to install the component, whatever kind of component it is.
