...
func (r *MarkdownRenderer) Entry(w io.Writer, e *tui.ComponentEntry) error {
	...
	if entry.Doc != "" {
		buf.WriteString("\n")
		writeDoc(&buf, entry.Doc, e.Component.IsMarkDown())
	}
...
func writeDoc(buf *bytes.Buffer, doc string, markdown bool) {
	fence := docFence(doc)

	buf.WriteString(fence)

	// A README that isn't Markdown (AsciiDoc, today) gets a bare fence rather
	// than an info string claiming a language it isn't written in.
	if markdown {
