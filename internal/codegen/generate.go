...
func shouldContinueWithFileExists(
	...
	case ExistsOverwriteTerragrunt:
		...
		if !wasGenerated {
			l.Errorf(
				"ERROR: The file path %s already exists and was not generated by terragrunt.",
				path,
			)
...
func shouldRemoveWithFileExists(
	...
	case DisabledRemoveTerragrunt:
		...
		if !wasGenerated {
			...
			return false, GenerateFileRemoveError{path: path}
		}
...
func RemoteStateConfigToTerraformCode(
	...
	for _, key := range backendKeys {
		...
		if key == assumeRoleWithWebIdentityConfigKey {
			...
			err := hclsimple.Decode(
				"s3_assume_role_with_web_identity.hcl",
				[]byte(hclValue),
				nil,
				&parsedConfig,
			)
	...
	if !found {
		return nil, errors.New(
			EncryptionKeyProviderKey + " is mandatory but not found in the encryption map",
		)
	}
...
func convertValue(v any) (ctyjson.SimpleJSONValue, error) {
	...
	if err != nil {
		return ctyjson.SimpleJSONValue{}, err
	}
